...
- Per-origin and global rate limits with increasing delay; never a permanent lock
...
- After login or `/session/me`, every authenticated administrative request sends
  ...
  reload, the previous hash is accepted for at most 60 seconds. GET mutations are
  not accepted.
