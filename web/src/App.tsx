...
function Brand() { return <a className="brand" href="/" aria-label="ModelCairn, inicio"><span className="brand-mark" aria-hidden="true"><i/><i/><i/></span><span>ModelCairn</span></a>; }
...
function Console({ session, online, onLogout }: { session: SessionContext; online: boolean; onLogout: () => void }) {
  ...
  const [page, setPage] = useState<"overview" | "resources" | "secrets">("overview");
  ...
  return <div className="app-shell">
    ...
    <aside aria-label="Navegación principal"><nav><button className={page==="overview"?"active":""} onClick={() => setPage("overview")}>Resumen</button><button onClick={() => setOnboarding(true)}>Crear primera ruta</button><button className={page==="resources"?"active":""} onClick={() => setPage("resources")}>Recursos</button><button className={page==="secrets"?"active":""} onClick={() => setPage("secrets")}>Secretos</button><button disabled>Actividad</button><button disabled>Configuración</button></nav><a className="repo-link" href="https://github.com/sxamx/modelcairn" rel="noreferrer">ModelCairn · Código abierto</a></aside>
    <main className="content" id={page}>{page === "resources" ? <Resources/> : page === "secrets" ? <Secrets/> : <><p className="eyebrow">RESUMEN</p><h1>Hola, {session.admin.username}</h1><p className="lede">Una vista privada del estado y la actividad de esta instalación.</p>{unavailable && <div className="form-error" role="status">El resumen no está disponible. Tus rutas siguen funcionando de forma independiente.</div>}
