...
function RouteDetail({ id, data, stepsFor, onSaved }: { id: string; data: CatalogData; stepsFor: (route: Resource) => Resource[]; onSaved: () => Promise<void> }) {
  ...
  return <>
    ...
    <div className="route-page-head"><div><p className="eyebrow">RECORRIDO DE RESPALDO</p><h1 id="catalog-title">Ruta {alias}</h1><p>{editing ? "Abre un destino para cambiar su prioridad. Guarda el borrador y publícalo cuando esté listo." : "Toca un destino para ver sus datos. El tráfico usa la última versión publicada."}</p></div><span className={enabled(route) ? "catalog-badge ready" : "catalog-badge"}>{enabled(route) ? "Habilitada" : "Deshabilitada"}</span></div>
    ...
    <RouteFlow alias={alias} layoutId={id} steps={flow} selected={dialogId ?? ""} onSelect={setDialogId} onSourceClick={() => setSourceOpen(true)}
