...
export default function Onboarding({ onClose, onComplete }: Props) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const plan = await api.planConfiguration(configuration, secretName);
  ...
  async function apply() { if(!pending)return; setBusy(true);setError("");let current=pending;try{if(!current.configured){await api.createSecret(current.secretName,current.secret);try{await api.applyConfiguration(current.configuration,current.plan.planToken);}catch(reason){const refreshed=await api.planConfiguration(current.configuration);if(refreshed.changes.length){setPending({...current,plan:refreshed});throw reason;}}current={...current,configured:true};setPending(current);}try{const issued=await api.issueAgentToken(current.agentName);complete(issued.token,current);}catch(reason){const status=await api.agentTokenStatus(current.agentName);if(status.tokenStatus.state==="active"){setPending({...current,tokenRecovery:true});setError("La ruta quedó creada, pero el token ya fue emitido y su respuesta se perdió. Rótalo para mostrar un token nuevo y continuar.");return;}throw reason;}}catch(reason){const code=reason instanceof APIError?reason.code:"request_failed";setError(code==="precondition_required"?"Ese secreto ya existe y no fue reemplazado. Elige otro identificador.":`No se pudo completar el asistente (${code}). Revisa el estado y vuelve a intentarlo; la API key guardada no se pierde.`);}finally{setBusy(false);} }
  async function replaceToken(){if(!pending)return;setBusy(true);setError("");try{const issued=await api.rotateAgentToken(pending.agentName);complete(issued.token,pending);}catch(reason){const code=reason instanceof APIError?reason.code:"request_failed";setError(`No se pudo rotar el token (${code}). Comprueba su estado antes de reintentar.`);}finally{setBusy(false);}}
