...
startup_time_file="$work_directory/startup.time"
server_pid=""
cleanup() {
...
"$binary" config validate docs/contratos/config/example-v1alpha1.yaml >>"$command_log"
"$binary" config plan --data-dir "$data_directory" --out "$plan_file" docs/contratos/config/example-v1alpha1.yaml >>"$command_log"
"$binary" config apply --data-dir "$data_directory" --plan "$plan_file" docs/contratos/config/example-v1alpha1.yaml >>"$command_log"
"$binary" config export --data-dir "$data_directory" >"$export_file"
"$binary" config validate "$export_file" >>"$command_log"
"$binary" config plan --data-dir "$data_directory" --out "$noop_plan_file" "$export_file" >>"$command_log"
"$binary" config apply --data-dir "$data_directory" --plan "$noop_plan_file" "$export_file" >>"$command_log"
...
printf '%s\n' "$startup_milliseconds" >"$startup_time_file"

# A stateful offline command must lose cleanly while the service owns the installation.
if "$binary" config export --data-dir "$data_directory" >>"$command_log" 2>&1; then
...
"$binary" config export --data-dir "$data_directory" >"$work_directory/export-after.json"
